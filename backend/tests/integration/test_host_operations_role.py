...
def test_host_operations_role_only_uses_fixed_user_operations() -> None:
    ...
    assert "host_operation_effective_home" in content
    assert "Remove managed sudo policy when deleting local user" in content
    assert "Remove managed sudo policy when an update clears it" in content
    assert "state: absent" in content
    assert "getent_passwd[host_operation.username][4]" in content
...
def test_sshd_role_validates_before_reloading_service() -> None:
    ...
    assert "KbdInteractiveAuthentication" in content
    assert "99-server-account-console.conf" in content
    assert "sshd -T" in content
    assert "sshd -t" in content
    assert "check_mode: false" in content

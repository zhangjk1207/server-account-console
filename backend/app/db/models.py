...
class HostAccessGrant(TimestampedRecord, Base):
    ...
    account_origin: Mapped[str] = mapped_column(String(16), default="created")
    remote_uid: Mapped[int | None] = mapped_column(Integer, nullable=True)
    remote_primary_group: Mapped[str | None] = mapped_column(String(64), nullable=True)
    remote_home: Mapped[str | None] = mapped_column(String(255), nullable=True)
    managed_public_keys: Mapped[list[str]] = mapped_column(JSON, default=list)
    managed_key_fingerprints: Mapped[list[str]] = mapped_column(JSON, default=list)
    data_directory: Mapped[str | None] = mapped_column(String(255), nullable=True)

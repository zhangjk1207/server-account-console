...
test("admin previews and confirms a member-centric access grant", async ({ page }) => {
  let loggedIn = false;
  const job = { id:"job-1", kind:"access_grant_provision", state:"ready_to_confirm", created_at:"2026-07-16T00:00:00Z", request_snapshot:{ grants:[] }, targets:[] };
  ...
  await page.getByLabel("我已核对本批次变更").check();

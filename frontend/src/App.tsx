...
function HostDetail({host,users,onClose,onError,onPreview}:{host:Host;users:User[];onClose:()=>void;onError:(message:string)=>void;onPreview:(job:Job)=>void}) {
  ...
  const previewUserOperation = async (event:FormEvent<HTMLFormElement>) => {
    ...
    if (action === "upsert") { if (groups.trim()) payload.groups = groups.split(",").map((value) => value.trim()).filter(Boolean); if (shell.trim()) payload.shell = shell.trim(); if (home.trim()) payload.home = home.trim(); if (sudoRule.trim()) payload.sudo_rule = sudoRule; }
    if (action === "delete") payload.remove_home = removeHome;
    if (action === "keys") { payload.key_mode = keyMode; payload.public_keys = keys.split("\n").map((value) => value.trim()).filter(Boolean); }

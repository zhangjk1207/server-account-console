...
import { api, Host, HostUserState, Job, JobEvent, login, Script, SshKey, User } from "./api";
...
const stateLabel: Record<string, string> = { pending:"等待中", preview_running:"预检中", ready_to_confirm:"预检完成", preview_failed:"预检失败", running:"执行中", succeeded:"已完成", partial_failed:"部分失败", expired:"已过期", failed:"失败", synced:"已同步", enabled:"已启用", disabled:"已停用" };
...
function Hosts({hosts,onAdd,onArchive,onTest}:{hosts:Host[];onAdd:(value:Record<string,unknown>)=>Promise<void>;onArchive:(host:Host)=>Promise<void>;onTest:(host:Host)=>Promise<void>}) { return <><InlineForm title="接入机器" fields={["name:名称","address:IP 或 FQDN","port:端口","ssh_user:SSH 用户","tags:标签，逗号分隔"]} onSubmit={async (form) => onAdd({name:form.name,address:form.address,port:Number(form.port || 22),ssh_user:form.ssh_user || "root",tags:(form.tags || "").split(",")})}/><Table rows={hosts} columns={["name","address","ssh_user","tags"]} render={(host) => <><State state={host.status}/>{host.last_probe_error && <small className="muted">{host.last_probe_error}</small>}</>} action={(host) => <><Button onClick={() => void onTest(host)}>测试连接</Button><Button className="quiet" onClick={() => void onArchive(host)}>归档</Button></>}/></>; }
